...
CDeHalo::CDeHalo(PClip _child, PClip _bClip, int _nt, int _mode, bool _Y, bool _U, bool _V, IScriptEnvironment* env) : GenericVideoFilter(_child), bClip(_bClip), nt(_nt), mode(_mode), Y(_Y), U(_U), V(_V)
{
    if (!vi.IsY8() && !vi.IsYV12() && !vi.IsYV16() && !vi.IsYV24())
    {
        env->ThrowError("CDeHalo: supported colorspaces are Y8, YV12, YV16, YV24!");
    }
    else if (nt < 0 || nt > 255)
    {
        env->ThrowError("CDeHalo: nt avaible range is [0, 255]!");
    }
    else if (mode < 0 || mode > 2)
    {
        env->ThrowError("CDeHalo: mode avaible mode values are 0, 1, 2!");
    }
    else if (!bClip)
    {
        env->ThrowError("CDeHalo: a blur clip must be specified!");
    }

}

PVideoFrame __stdcall CDeHalo::GetFrame(int n, IScriptEnvironment* env) {
